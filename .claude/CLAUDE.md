...
4. **Library** (`.claude/lib/`) - Reusable bash functions for common operations

### Common Functions Library

**Location**: `.claude/lib/common-functions.sh`
**Purpose**: Eliminate code duplication across command files with reusable bash functions
**Version**: 1.0.0

**Available Functions**:
- `get_workspace_progress(TASK_ID, WORKSPACE_DIR)` - Calculate checklist completion percentage
- `update_task_status(TASK_ID, STATUS, CSV_FILE)` - Update task status in tasks.csv
- `update_checklist_item(FILE, TEXT, STATUS)` - Mark checklist items complete/incomplete
- `append_note(TEXT, FILE, FORMAT)` - Add timestamped notes to notes.md
- `git_commit_with_message(MESSAGE, FILES, TASK_ID)` - Create standardized git commits
- `format_duration(SECONDS)` - Format seconds to human-readable duration (e.g., "2h 15m 30s")

**Usage Example**:
```bash
# Source the library in your command
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/common-functions.sh"

# Use functions
PROGRESS=$(get_workspace_progress "TASK-001")
update_task_status "TASK-001" "completed"
```

**Documentation**: See `.claude/lib/README.md` for detailed function documentation, examples, and migration guide.

**Test Coverage**: 100% - All functions have comprehensive unit and integration tests in `.claude/tests/test_common_functions_scaffold.sh`
